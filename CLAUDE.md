...
- **Golden Awards winner is locked** — `GOLDEN_AWARDS_WINNER` constant in `GithubDashboard.gs` (currently `"VicV"`) overrides `stageLeaders.goldenAwards` once a live winner exists (`liveGoldenAwardsWinner !== null`). Same rationale as the other locks: the tiebreak (lowest total points) is unstable while everyone's total points keep climbing. Update this constant manually if the locked winner is ever wrong.
